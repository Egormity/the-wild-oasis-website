    ...
    <nav className='h-full border-r border-primary-900'>
      <ul className='flex h-full flex-col gap-2 text-lg'>
        {navLinks.map(link => (
          <li key={link.name}>
            <Link
              className={`${pathName === link.href ? 'bg-primary-900' : ''} flex gap-4 px-5 py-3 font-semibold text-primary-200 transition-colors hover:bg-primary-900 hover:text-primary-100`}
              href={link.href}
            >
              {link.icon}
              <span>{link.name}</span>
